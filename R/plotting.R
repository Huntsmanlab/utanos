...
ACNDiversityPlot <- function(long_segments = data.frame(),
  ...
  if (isTRUE(order)) {
    ...
    long_segments$chromosome <- factor(long_segments$chromosome, levels = c(1:22, "X", "Y"))
    long_segments$sample_id <- factor(long_segments$sample_id, levels = order)
  } else {
...
#' @param cluster optional *logical* Do a row-wise sort of the heatmap (sort the samples)
...
#'
#' @export
RCNDiversityPlot <- function(qdnaseq_obj,
                             order_by = NULL,
                             cluster = TRUE,
                             subset = NULL,
                             Xchr = FALSE,
  ...
  if (!is.null(rmblacklist)) {
    ...
    if (rmblacklist == 'hg19') {
      long_segs = RemoveBlacklist(long_segs, refgenome = 'hg19')
    } else if (rmblacklist == 'hg38') {
      long_segs = RemoveBlacklist(long_segs, refgenome = 'hg38')
    } else {
      stop("Invalid value for rmblacklist parameter.")
    }
  }
  ...
  colnames(long_segs) <- c("chromosome", "start", "end", "segmented", "sample_id")
